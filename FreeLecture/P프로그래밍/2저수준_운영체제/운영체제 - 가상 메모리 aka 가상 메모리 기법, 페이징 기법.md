...
# 2. 3줄 요약
- 1. 가상 메모리는 메모리를 안전하고 효율적으로 사용하기 위해 존재하는 추상화된 메모리 공간이다.
- 2. MMU는 다양한 방법으로 가상 메모리의 주소를 물리적 메모리 주소로 바꿔준다.
- 3.
# 3. 이해를 위한 사전 지식(빌드업)
- [[컴퓨터 구조 - CPU]]
...
- 운영체제는 가상 메모리 중에서 현재 사용되지 않는다고 판단되는 메모리 조각들(페이지라고 함)을 스왑하여 실제 메모리 공간을 확보하는 등의 관리를 함.
	- 메모리를 압축한다는 설도 있으나 해당 내용에 대해선 추가 조사가 필요. #추가조사필요 
		- 예전에 GC에서 배웠던, 빈 공간이 없어질때까지 한쪽으로 밀어넣는건지는 확인이 필요. 그때도 압축이라는 표현을 사용했음.
		- 예전에 GC에서 배웠던, 
...
https://sommda.tistory.com/63
![[Pasted image 20250119201703.png]]
https://lordofkangs.tistory.com/626?pidx=4
#### 4.1.3.1.4. 역 매핑
...
#### 4.1.3.1.5. 페이지 테이블의 위치
##### 4.1.3.1.5.1. CPU 내부의 레지스터에 위치한 경우
- CPU 내부에 페이지 테이블을 만들면, 주소 변환 속도가 빠르다.
- CPU 내부에 사용할 수 있는 레지스터는 한정되어 있으므로 페이지 테이블의 크기가 매우 제한된다.
##### 4.1.3.1.5.2. 메모리 내부에 위치한 경우
- 페이지 테이블의 크기에 제한이 없어진다
- 주소 변환 속도가 느리다.
	- CPU에는 레지스터가 메모리보다 가깝다.
##### 4.1.3.1.5.3. TLB(Translation Look-aside Buffer)
- 페이지 테이블을 CPU 레지스터나, 혹은 메모리에 위치시킬 때, 장점과 단점이 너무 확실해서 그 중간 정도를 선택했다.
- 페이지 테이블을 저장할 별도의 칩을 만들고, CPU와 메모리 사이에 위치시켜서 해결했다.
...
- 프로세스는 페이지의 집합
- CPU가 어떤 페이지를 필요로 할때, 그 요구되는 페이지만 페이지를 메모리에 올리는 것이다
...
#### 교체정책 / Victim Page
- 어떤 페이지를 교체할 것인가에 대한 내용. 
- i/o 시간 절약을 위해 진행함,
- 가까운 미래에 참조될 가능성이 가장 적은 페이지어야 한다.
- 지역성의 원리를 이용한다.
	- 메모리 접근은 시간적, 공간적 지역성을 가진다.
##### LRU (Least Recently Used)
가장 오랫동안 참조되지 않은 페이지를 교체한다.
##### FIFO (First In First Out)
가장 오래 전에 반입 된 페이지는 사용하지 않을거라 판단함.

그 외 추가적인 페이지 교체 알고리즘
#추가조사필요 
- OPT
- LFU
- MFU
...
- 대표적으로 페이지 테이블마다 r(read), w(write), x(execute) 비트를 두어, 해당 비트가 켜져있을 때 그 수행이 가능하도록한다.
	- 예를 들어, 쓰기 비트가 꺼져있는 페이지에 쓰기 작업을 시도하면 CPU에 인터럽트가 발생하여 ISR에서 강제로 해당 프로세스를 종료시킨다.
![[Pasted image 20250119202634.png]]


공유는 메모리 낭비를 방지하기 위함이다.
- 프로세스의 메모리는 code + data + stack 영역으로 나뉜다.
- 같은 프로그램을 쓰는 복수 개의 프로세스가 있다면, code 영역은 같을 것이다.
- 그러므로 하나의 code 영역을 복수 개의 프로세스가 공유하여 메모리 낭비를 줄이는 것이다.
	- 단, code가 공유되려면 code가 변하지 않는 프로그램이어야 한다.
	- 이를 non-self-modifying code = reentrant code(재진입가능 코드) = pure code 라고 한다
